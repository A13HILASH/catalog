...
// Book reader modal for markdown content
const BookReaderModal = ({ open, onClose, bookUrl, title }) => {
        ...
        <div className="bg-gradient-to-r from-slate-50 to-slate-100 p-6 border-t border-slate-200">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2 text-slate-600">
              ...
              <span className="text-sm">Reading from Cloudinary</span>
            </div>
            
            <button 
              onClick={onClose} 
              className="px-6 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-medium rounded-xl transition-all duration-200 transform hover:scale-105 shadow-lg shadow-emerald-500/25 flex items-center space-x-2"
            >
...
export default function BookDetailModal({ open, handleClose, book }) {
  ...
  // Check if this book has a bookUrl (Cloudinary markdown content)
  const hasBookUrl = info.bookUrl && info.bookUrl.includes('cloudinary.com');
  const previewUrl =  info.previewLink || hasBookUrl;
  console.log(previewUrl);
  console.log(info.previewLink);

  const handlePreviewClick = () => {
    if (hasBookUrl) {
      // Open the book reader modal for markdown content
      setReaderModalOpen(true);
    } else if (previewUrl) {
      // Open Google Books preview in new tab using the ID
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-2 text-slate-600">
                ...
                <span className="text-sm">
                  {hasBookUrl ? 'Full book available to read' : 'Book information from Google Books'}
                </span>
              </div>
                  ...
                  <button 
                    ...
                    className={`px-6 py-2 font-medium rounded-xl transition-all duration-200 transform hover:scale-105 shadow-lg flex items-center space-x-2 ${
                      hasBookUrl 
                        ...
                        : 'bg-gradient-to-r from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 text-white shadow-slate-500/25'
                    }`}
                  >
                    {hasBookUrl ? (
                      <>
